...
CodeMirror.defineMode("kotlin", function (config, parserConfig) {
  ...
  var keywords = words(
          ...
          " when tag this else This try val var fun for is in if do as true false null get set");
  ...
  function Context(indented, column, tag, align, prev) {
  ...
  function pushContext(state, col, tag) {
    return state.context = new Context(state.indented, col, tag, null, state.context);
  }

  function popContext(state) {
    var t = state.context.tag;
    if (t == ")" || t == "]" || t == "}")
  ...
  return {
    ...
    token: function (stream, state) {
      ...
      if (stream.sol()) {
        ...
        if (ctx.tag == "statement" && !expectExpression(state.lastToken)) {
          popContext(state);
          ctx = state.context;
        }
      }
      ...
      if ((curPunc == ";" || curPunc == ":") && ctx.tag == "statement") popContext(state);
      // Handle indentation for {x -> \n ... }
      else if (curPunc == "->" && ctx.tag == "statement" && ctx.prev.tag == "}") {
        popContext(state);
        ...
        while (ctx.tag == "statement") ctx = popContext(state);
        if (ctx.tag == "}") ctx = popContext(state);
        while (ctx.tag == "statement") ctx = popContext(state);
      }
      else if (curPunc == ctx.tag) popContext(state);
      else if (ctx.tag == "}" || ctx.tag == "top" || (ctx.tag == "statement" && curPunc == "newstatement"))
    ...
    indent: function (state, textAfter) {
      ...
      if (ctx.tag == "statement" && !expectExpression(state.lastToken)) ctx = ctx.prev;
      var closing = firstChar == ctx.tag;
      if (ctx.tag == "statement") {

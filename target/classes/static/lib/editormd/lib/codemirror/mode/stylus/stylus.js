...
(function(mod) {
  ...
  CodeMirror.defineMode("stylus", function(config) {
    ...
    var tokenBase = function(stream, state) {
      ...
      if (stream.match(/^\$?[a-z][\w-]+\s?=(\s|[\w-'"\$])/i)) {
        ...
        var cssTagSelector = stream.current().toLowerCase().match(/[\w]+/)[0];
        if(cssTagSelectors[cssTagSelector] === undefined) {
          return "variable-2";
      ...
      if (stream.match(/[\w-]*/i)) {
        ...
        } else if(cssTagSelectors[word] !== undefined) {
          // CSS tag selectors
          indent(state);
          return "tag";
  ...
  var wordOperators = ["in", "and", "or", "not", "is a", "is", "isnt", "defined", "if unless"],
      ...
      commonAtoms = ["null", "true", "false", "href", "title", "tag", "not-allowed", "readonly", "disabled"],
      commonDef = ["@font-face", "@keyframes", "@media", "@viewport", "@page", "@host", "@supports", "@block", "@css"],
      cssTagSelectors = keySet(cssTagSelectors_),
